    ...
    public class ProductTransactionRepository : IProductTransactionRepository
    {
            ...
            ProductTransactionType? transactionType)
        {
            ...
            var query = from pt in db.ProductTransactions
                        ...
                        where (string.IsNullOrWhiteSpace(productName) || prod.ProductName.Contains(productName, StringComparison.OrdinalIgnoreCase)) &&
        ...
        public async Task ProduceAsync(string productionNumber, Product product, int quantity, double price, string doneBy)
        {
            ...
            if (prod != null)
            {
                foreach (var pi in prod.ProductInventories)
                {
                    ...
                    this.db.InventoryTransaction.Add(new InventoryTransaction
                    {
                        ProductionNumber = productionNumber,
                        InventoryId = pi.Inventory.InventoryId,
                        QuantityBefore = qtyBefore,
                        ActivityType = InventoryTransactionType.ProduceProduct,
                        QuantityAfter = pi.Inventory.Quantity + quantity,
